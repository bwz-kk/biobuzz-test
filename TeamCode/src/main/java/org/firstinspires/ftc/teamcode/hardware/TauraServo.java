...
public class TauraServo {
    ...
    private double lastCommandedPosition = Double.NaN;
    ...
    /**
     * Returns where on the servo's full pulse range the last command actually lands, in [0..1],
     * after applying direction and range scaling. Unlike {@link #getPosition()}, two servos with
     * different direction/range settings report different values here even for the same logical
     * position, so this is the number to compare when one servo seems to travel further than the
     * other. Returns NaN before the first {@link #setPosition(double)}.
     */
    public double getPulsePosition() {
        if (Double.isNaN(lastCommandedPosition)) return Double.NaN;
        double clipped = Math.max(0.0, Math.min(1.0, lastCommandedPosition));
        double directed = reversed ? 1.0 - clipped : clipped;
        return rangeMin + directed * (rangeMax - rangeMin);
    }

    /**
     * Scales the logical range of motion without changing the servo type in Robot Configuration.
     * Useful to constrain movement to a safer subrange. Ignored if {@code min >= max}.
     */
    ...
    public void setDirection(Servo.Direction direction) {
        reversed = direction == Servo.Direction.REVERSE;
